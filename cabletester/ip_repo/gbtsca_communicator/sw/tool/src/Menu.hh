...
public:
  ...
  class Line {
  public:
    Line(const char* n, const char* d, void (*f)( target_T* target ), bool hidden=false) : name(n),desc(d),subMenu(0),func(f),func2(0), isNull(false),isHidden(hidden) { }
    Line(const char* n, const char* d, void (*f2)( const std::string& cmd, target_T* target ), bool hidden=false) : name(n),desc(d),subMenu(0),func(0),func2(f2), isNull(false),isHidden(hidden) { }
    Line(const char* n, const char* d, Menu* sb , bool hidden=false) : name(n),desc(d),subMenu(sb), func(0),func2(0), isNull(false),isHidden(hidden) { }
    Line(const char* n, const char* d, bool hidden=false) : name(n),desc(d),subMenu(0), func(0),func2(0), isNull(false),isHidden(hidden) { }
    Line() : isNull(true) { }
    ...
    const char* desc;
    Menu* subMenu;
    void (*func)( target_T* target ) ;
    void (*func2)( const std::string& cmd, target_T* target ) ;
  private:
    bool isNull;
    bool isHidden;
  };
