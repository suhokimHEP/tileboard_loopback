...
class HDLCzynqMP : public hgcsca::HDLC {
public:
  HDLCzynqMP();
  ~HDLCzynqMP();
...
IMPLEMENT_HDLC_LOADER(HDLCzynqMP,zynqMP);

HDLCzynqMP::HDLCzynqMP() {
  sequence_=1;
}
...
bool HDLCzynqMP::reset() {
  send_reset();
  sequence_=1;
  return true;
}

bool HDLCzynqMP::resetHDLC() {
  reset_HDLC();
  return true;
}

hgcsca::HDLC::Response HDLCzynqMP::requestSync(uint8_t channel, uint8_t reg, uint8_t len, uint32_t data) {
...
hgcsca::HDLC::Response HDLCzynqMP::getResponse() {
...
void HDLCzynqMP::configure(const std::string& uri) {
  configure_mmap();
}
